...
//    ];
    ...
    useEffect(() => {
        setFilteredTasks(tasks.filter((task) => {
            ...
            if (selectedFilter === 'pendingPositive') return (task.status !== 0 && task.etat === 'En cours de traitement');
            if (selectedFilter === 'treatedPositive') return (task.status !== 0 && (task.etat !== 'En cours de traitement' && task.etat !== 'En attente de traitement' && task.etat !== 'Pas de traitement à faire'));
            ...
            <tr key={task.screeningId} onClick={() => handleRowClick(task.screeningId)} className="row-hover">
                <td style={{ width: "28%" }}>
                    {selectedFilter !== 'treatedPositive' ?
                        (task.status != 0 ? 'Controle d\'un Hit positif' : 'Hit negatif') :
                        task.resultStatus}
                </td>
                    ...
                    <div className="row mx-1">
                        <div className="total-rows col-md-12 ">
                            ...
                            <span className={`green-line-screening ${selectedFilter === 'pendingN' ? 'selected' : ''}`}
                                ...
                                Hits positifs en attente de traitement niveau N  ({tasks.filter(task => task.status !== 0 && task.etat === 'En attente de traitement').length})
                            </span>
                            {'   |   '}
                            <span className={`green-line-screening ${selectedFilter === 'pendingPositive' ? 'selected' : ''}`}
                                onClick={() => handleFilterChange('pendingPositive')}>
                                Hits positifs en attente de traitement niveau N+1 ({tasks.filter(task => task.status !== 0 && task.etat === 'En cours de traitement').length})
                            </span>
                            {'   |   '}
                            <span className={`green-line-screening ${selectedFilter === 'treatedPositive' ? 'selected' : ''}`}
                                onClick={() => handleFilterChange('treatedPositive')}>
                                Hits positifs trait&eacute;s ({tasks.filter(task => task.status !== 0 && (task.etat !== 'En cours de traitement' && task.etat !== 'En attente de traitement' && task.etat !== 'Pas de traitement à faire')).length})
                            </span>
                            {'   |   '}
                            ...
                            <table className="table custom-table table-hover">
                                <thead>
                                    <tr>
                                            ...
                                            {selectedFilter === 'all' && <th scope="col">Status</th>} {/* Conditional column */}
                                    </tr>
                                </thead>
                                <tbody>
                                    {renderTableRows()}
